...
bool create(FILE * input, Country **data, size_t *size)
{
	...
	{
		if (*data = malloc(*size * sizeof(Country)))
		{
...
bool fill(FILE * input, Country *const data, size_t size)
{
	...
	printf("Enter square (real), population (integer) and capital (word) %ld times\n", size);
	while ((size > i) && (fscanf(input, "%f %d %254s", &data[i].square, &data[i].population, temp_capital) == 3))
	{
		if (data[i].capital = malloc(strlen(temp_capital) * sizeof(char)))
		{
...
void show(FILE *output, const Country *const data, size_t size)
{
	...
	for (size_t i = 0; i < size; ++i)
	{
		fprintf(output, "%14.2f %14d %18s %13.3f\n", data[i].square, data[i].population, data[i].capital, data[i].population / data[i].square);
	}
}
...
int partition(Country *const data, size_t size)
{
	...
	{
		if ((data[0].population / data[0].square)  > (data[j].population / data[j].square))
		{
			swap(&data[i], &data[j]);
			++i;
		}
